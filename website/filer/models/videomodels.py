#-*- coding: utf-8 -*-
from PIL import Image as PILVideo
...
class Video(File):
    SIDEBAR_Video_WIDTH = 210
    DEFAULT_THUMBNAILS = {
        'admin_clipboard_icon': {'size': (32, 32), 'crop': True,
                                 'upscale': True},
        'admin_sidebar_preview': {'size': (SIDEBAR_Video_WIDTH, 10000)},
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            self.file.seek(0)
            self._width, self._height = PILVideo.open(self.file).size
        except Exception:
            # probably the Video is missing. nevermind.
            pass
    ...
    def sidebar_Video_ratio(self):
        if self.width:
            return float(self.width) / float(self.SIDEBAR_Video_WIDTH)
        else:
            return 1.0
    ...
    def has_generic_permission(self, request, type):
        ...
        Video. Return the string 'ALL' if the user has all rights.
        """
        user = request.user
    ...
    def thumbnails(self):
        _thumbnails = {}
        for name, opts in Video.DEFAULT_THUMBNAILS.items():
            try:
                ...
                # to the template {{ Video.icons.48 }}
                pass
        return _thumbnails
